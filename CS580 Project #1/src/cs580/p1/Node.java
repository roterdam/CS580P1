...
final class Node {
   ...
   public static final int         SIMPLE_NODE = 0, IF_NODE = 1,
         WHILE_NODE = 2, DO_NODE = 3, RETURN_NODE = 4, DUMMY_NODE = 5,
         ELSE_NODE = 6;

   int                             firstLineNumber = -1, lastLineNumber = -1;
   Node                            exitNode        = null, controlNode = null;
   /*
   ...
   boolean                         processElse     = false;                   // Need
                                                                               // for
                                                                               // the
                                                                               // if-else
                                                                               // case.
   ...
   public boolean addEdge(Node towards) {
       ...
       * implementation is later changed, then it will be necessary to make sure
       * that nodes are being added to the end of the list.
       */
   ...
   public void setExit(Node n) {
      if (DEBUG)
         System.out.println("DEBUG: setExit was called. Setting Node: " + UUID
               + " exit to node: " + n.UUID);
      switch (this.type) {
         case Node.DO_NODE:
            /*
         ...
         case Node.IF_NODE:
            if (DEBUG)
               System.out.println("NODE:" + this.UUID
                     + " type is IF, edgeList.size()=" + edges.size());
            if (edges.size() == 2) {
               // This is an if-then-else node.
               Node left = edges.get(0);
               Node right = edges.get(1);
               if (DEBUG)
                  System.out.println("left: " + left.UUID
                        + " is relinking exit to node: " + n.UUID);
               left.setExit(n);
               if (DEBUG)
                  System.out.println("right: " + right.UUID
                        + " is relinking exit to node: " + n.UUID);
               right.setExit(n);
               if (DEBUG)
                  System.out.println("setExit finished for IF type node: "
                        + this.UUID);
            } else {// Just a regular if-then node.
               if (DEBUG)
                  System.out.println("Setting " + this.UUID + " exit to "
                        + n.UUID);
         ...
         default:
            if (DEBUG) {
               System.out.println("Through the switch statement, node: " + UUID
                     + " type:" + type);
            }
            /*
             * The only things that should fall in here are: Simple, Dummy, and
             * Return. Realistically, Dummy and return nodes should never be
             * processed through here, as that means there has been a mistake
             * and this requires a error message and a rage quit.
             */
            ...
            if (DEBUG)
               System.out.println("Not a Dummy nor a return either, "
   ...
   public boolean nesting(Node nest) {
      ...
      switch (type) {
         case IF_NODE:
            ...
            } else if (this.processElse) { // Statements are being added to the
                                           // else branch.
               // exitNode=null;
               edges.remove(1);
               edges.add(1, nest);
         ...
         case DO_NODE:
             ...
             * control node of the do loop. But we can't discard the head of the
             * do-loop as things may be pointing to it. So, we are going to link
             * the head of the do (making it a dummy node if you will) to this
             * new structure.
             */
            Node control = this.exitNode;
            if (DEBUG) {
               System.out.println("Linking this (" + UUID + ","
                     + this.firstLineNumber + "," + this.lastLineNumber + ")"
                     + "to (" + nest.UUID + "," + nest.firstLineNumber + ","
                     + nest.lastLineNumber + ") and that to " + "("
                     + control.UUID + "," + control.firstLineNumber + ","
                     + control.lastLineNumber + ")");
            }
            this.exitNode = nest;
         ...
         default:
            ...
            System.err.println("Error when trying to nest.\n\tCurrent node is"
                  + "Is: " + this.type
                  + " and cannot be nested with a passed node" + " of type: "
                  + nest);
            System.exit(-2);
      }
      return success;
      ...
      if (visited.contains(this)) {
         if (DEBUG)
            System.out.println("Node: " + UUID
                  + " was already visited, returning...");
         return new StringBuilder(""); // Already processed.
      }
      if (DEBUG)
         System.out.println("Adding Node: " + UUID + " to the visited list.");
      visited.add(this);
      StringBuilder buffer = new StringBuilder();

      buffer.append("\nNode: " + UUID + " lines: " + this.firstLineNumber
            + " to " + lastLineNumber + " type: " + type + "\n");
