 ...
 *         
 *         Assumptions: 1) There aren't any single line
 *         statements, that is all statements following any control statement is
 *         in a statement block. 2) Close brace cannot be followed by an open
 *         brace on the same line. 3) The while for do-loops must occur on the
 *         same line as the close brace ending the do body.
 *         
 *  This program does not fulfill all of the objectives set out by the project.
 *         1) Above assumptions should not be required.
 */
public class ControlFlowGraph {
   ...
   Node                           graph                = null;
   /*
    * The stack will hold the last non-completed predicate node, with the hope
    * that this can be used to make the correct connections for the various
    * control flow objects. This should hopefully simplify nesting situations.
    */
    ...
    *           - String to be parsed.
    * @param lineNumber
    *           - lineNumber will be used when creating the nodes.
   ...
   public boolean parse(String line, int lineNumber) {
      ...
      if (DEBUG)
         System.out.println("whichCtrlStmt(\"" + str + "\") was called and"
         ...
         case Node.RETURN_NODE:
            ...
            if (graph == null) {
               ...
               for (; searcher.exitNode != null
                     && searcher.exitNode.type != Node.DUMMY_NODE; searcher = searcher.exitNode)
                  ;
            ...
            break;
         default:
            if (DEBUG) System.out.println("default case.");
            /*
             * So we need to check to see if the line contains an end of
             * statement token, ie '}'. If it does then the control structure is
             * done and can be popped off, otherwise we just assume that the
             * line is just a procedure.
             */
            if (str.contains(endControlToken)) {
               if (DEBUG)
                  System.out.println("Statement was an endControlToken");
               /*
               ...
               if (DEBUG)
                  System.out.println("Determined it to be a procedure"
                        + " node, calling process(" + Node.SIMPLE_NODE + ","
                        + lineNumber + "," + str + ")");
   ...
   private void closer(String str) {
      ...
      /*
       * Since the stack has elements, we know that the top has finished
       * processing. However, it could be the case that the we have a sequence
       * of elements. So What we are going to do is create a dummy node that
       * points to this finished node. During processing if we find that there
       * is a dummy node on the stack, we can link to it thus creating the
       * sequence.
       */
      if (DEBUG) {
         System.out.println("Stack Dump\nSize=" + lastControlStatement.size());
         for (Node n : lastControlStatement) {
            System.out.println("\tNode UUID: " + n.UUID + ", type: " + n.type);
         }
      ...
      if (stackPeek.type == Node.SIMPLE_NODE
            || stackPeek.type == Node.DUMMY_NODE) lastControlStatement.pop();
      ...
      for (int i = 0; i < edgeList.size(); i++) {
         ...
         if (edgeList.get(i).type == Node.DUMMY_NODE) {
            if (DEBUG)
               System.out.println("\tNode is a dummy node, performing"
                     + " relinking.");
      ...
      if (DEBUG)
         System.out.println("Popping node: " + popped.UUID + " of type: "
               + popped.type);
      ...
      if (DEBUG)
         System.out.println("Dummy node(" + dummy.UUID + ") was created "
               + "and push onto the stack.");
   }

   private void process(int nodeType, int lineNumber, String line) {
      if (DEBUG) {
         System.out.println("DEBUG: process(" + nodeType + "," + lineNumber
               + "," + line + ") was called."
         ...
         switch (lastAct.type) {
            ...
            case Node.SIMPLE_NODE:
               ...
               if (nodeType == Node.SIMPLE_NODE) {
                  if (DEBUG)
                     System.out.println("nodeType==SIMPLE.\nUpdating"
                           + "lastAct.lastLineNumber to: " + lineNumber);
                  lastAct.lastLineNumber = lineNumber;
               } else {
                  if (DEBUG)
                     System.out.println("nodeType (of passed node) "
                           + "!=SIMPLE: Executing else.");
                  Node P1 = lastControlStatement.pop();// Take P1 off of the
                                                       // stack
                  struct.setExit(P1.exitNode);
                  P1.setExit(struct); // link P1 to the new structure
                  if (graph == null) graph = P1; // If we start with a P1 then
                                                 // we need this.
                  lastControlStatement.push(struct);// push the new struct onto
                                                    // the stack
                  if (DEBUG) {
                     System.out.println("\nStack Dump:");
                     for (Node n : lastControlStatement) {
                        System.out.println("Node " + n.UUID + " type: "
                              + n.type);
                     }
                  }
               }
            ...
            case Node.DUMMY_NODE:
               ...
               if (DEBUG) {
                  System.out.println("\tPop Node: " + lastAct.UUID
                        + " of type: " + lastAct.type + " Lines: "
                        + lastAct.firstLineNumber + " to "
                        + lastAct.lastLineNumber + " and linking it to Node: "
                        + struct.UUID + " of type: " + struct.type + " lines: "
                        + struct.firstLineNumber + " to "
                        + struct.lastLineNumber);
               }
               if (struct.type == Node.ELSE_NODE) { // read in type: else, stack
                                                    // type: if
                  lastAct.nesting(struct);
                  struct = lastAct;
               } else {
                  if (DEBUG)
                     System.out.println("lastAct: " + lastAct.UUID + ","
            ...
            default:
               if (DEBUG)
                  System.out.println("on stack: UNKNOWN (default case)");
    ...
    * 
    * @param nodeType
    *           - int that should correspond to the values available in the node
    *           class.
    * @return returns a graph structure of the passed prime number. Invalid
    *         numbers return null.
    */
